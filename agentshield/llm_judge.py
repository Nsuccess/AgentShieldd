...
class LLMJudge:
    ...
    def __init__(
        ...
        if self.provider == "openai":
            ...
            # Groq - 10x faster than OpenAI, free tier available!
            self._api_key = api_key or os.getenv("GROQ_API_KEY")
            if self._api_key:
                try:
                    from groq import Groq
                    self._client = Groq(api_key=self._api_key)
                    self.logger.info("✅ Groq LLM initialized (10x faster than OpenAI!)")
                except ImportError:
                    self.logger.warning("Warning: groq package not installed. Run: pip install groq")
